...
import { BUYING_PROCESS, SEO, WHY_CHOOSE_US } from '@/content/siteContent'
    ...
    <>
        ...
        <div className="container-wide">
          ...
          <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-6">
            {BUYING_PROCESS.map((step, index) => (
              <SectionReveal key={step.title} delayMs={index * 50}>
                <div className="border border-light-tan/50 bg-off-white p-5 text-center">
                  <p className="font-display text-3xl text-gold">{String(step.step).padStart(2, '0')}</p>
                  <h3 className="mt-2 font-display text-xl text-primary">{step.title}</h3>
                  <p className="mt-2 text-sm text-leather">{step.description}</p>
                </div>
              </SectionReveal>
            ))}
          </div>
      ...
      <section className="section-pad bg-deep text-cream">
        <div className="container-narrow text-center">
          <h2 className="font-display text-4xl md:text-5xl">Ready to brief your corporate gifting program?</h2>
