      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {options.length ? (
          options.map((option) => (
            <span
              key={option.id}
              className="inline-flex items-center gap-1 border border-light-tan/80 bg-cream/50 px-2.5 py-1 text-sm text-primary"
            >
              {option.name}
              <button
                type="button"
          ...
          <span className="text-sm text-leather">No options yet.</span>
        )}
      </div>
